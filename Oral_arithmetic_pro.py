...
for i in range(num_formula):
    ...
    result = [0] * (2 * num_cal + 1)
    ans = [0]*num_cal
    step = 0

    for index, cal in enumerate(calculators_num):
        if cal > 2.5:  # 先算乘除
            result[index * 2 + 1] = calc_trans[cal]
            result[index * 2], result[index * 2 + 2], ans[step] = calculator(result[index * 2], cal)  # 乘除运算符后面的数一定没有生成呢
            step += 1

    for index, cal in enumerate(calculators_num):
        if cal < 2.5:  # 再算加减
            result[index * 2 + 1] = calc_trans[cal]
            result[index * 2], result[index * 2 + 2], ans[step] = calculator(result[index * 2], cal, result[index * 2 + 2])  # 加减运算符前后的数可能都生成过了
            step += 1
    result.append('=')
#    result.append(ans[-1])
